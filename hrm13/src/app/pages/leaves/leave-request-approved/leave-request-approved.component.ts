...
@Component({
  ...
  styleUrls: ['./leave-request-approved.component.css']
})
export class LeaveRequestApprovedComponent implements OnInit, AfterViewInit {
  ...
  async LoadAll(condition: any) {
    ...
    if (condition == 'hr') {
      await this.service.HrGetLeaveApproved(params).subscribe((resp: any) => {
        this.list.length = 0;
        this.list = resp;
      ...
      await this.service.ManagerGetLeaveApproved(params).subscribe((resp: any) => {
        this.list.length = 0;
        this.list = resp;
        this.dtElement.dtInstance.then((dtInstance: DataTables.Api) => {
          dtInstance.destroy();
