...
@Component({
  selector: 'app-leave-request-rejected',
  templateUrl: './leave-request-rejected.component.html',
  styleUrls: ['./leave-request-rejected.component.css']
})
export class LeaveRequestRejectedComponent implements OnInit, AfterViewInit {
  ...
  async LoadAll(condition: any) {
    ...
    if (condition == 'hr') {
      await this.service.HrGetLeaveRejected(params).subscribe((resp: any) => {
        this.list.length = 0;
        this.list = resp;
      ...
      await this.service.ManagerGetLeaveRejected(params).subscribe((resp: any) => {
        this.list.length = 0;
        this.list = resp;
        this.dtElement.dtInstance.then((dtInstance: DataTables.Api) => {
          dtInstance.destroy();
