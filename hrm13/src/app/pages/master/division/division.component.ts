...
import { ModalDivisionComponent } from 'src/app/components/modals/modal-division/modal-division.component';
...
export class DivisionComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  constructor(private page: PageDataService, private spinner: NgxSpinnerService,
    public dialog: MatDialog, private divisionService: DivisionService) {
    this.dtOptions = {
  ...
  ngOnInit(): void {
    this.page.pagename = "ຂໍ້ມູນຫນ່ວຍງານ"
    this.page.breadcrumb = '<li class="breadcrumb-item"> <i class="fas fa-home"></i> Home</li><li class="breadcrumb-item"> ການຈັດການຂໍ້ມູນພື້ນຖານ </li> <li class="breadcrumb-item active" > ຂໍ້ມູນຫນ່ວຍງານ </li>'
    // this.spinner.show();
    this.LoadAll();
  ...
  openDialog() {
    const dialogRef = this.dialog.open(ModalDivisionComponent, {
      width: '600px',
    });
  ...
  LoadAll() {
    ...
    this.divisionService.GetList().pipe(this.extractData).subscribe((resp: any) => {
  ...
  doDelete(id: any) {
    this.divisionService.Delete(id).subscribe((resp: any) => {
      this.spinner.hide()
      if (resp == "SUCCESS") {
        this.LoadAll();
      }
