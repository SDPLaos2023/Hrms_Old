...
import { ModalMaritalStatusComponent } from 'src/app/components/modals/modal-marital-status/modal-marital-status.component';
...
export class MaritalStatusComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    this.page.pagename = "ຂໍ້ມູນສະຖານະພາບ"
    this.page.breadcrumb = '<li class="breadcrumb-item"> <i class="fas fa-home"></i> Home</li><li class="breadcrumb-item"> ການຈັດການຂໍ້ມູນພື້ນຖານ </li> <li class="breadcrumb-item active" > ຂໍ້ມູນສະຖານະພາບ </li>'
    // this.spinner.show();
    this.LoadAll();
  ...
  LoadAll() {
    ...
    this.service.GetList().pipe(this.extractData).subscribe((resp: any) => {
      this.maritalstatus.length = 0;
      if (resp != null) {
        resp.forEach((element: any) => {
          if (element.id !== 'MR')
            this.maritalstatus.push(element)
        });
      }
  ...
  openDialog() {
    const dialogRef = this.dialog.open(ModalMaritalStatusComponent, {
      width: '600px',
    });
