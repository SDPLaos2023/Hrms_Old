...
import { ModalPositionComponent } from 'src/app/components/modals/modal-position/modal-position.component';
...
export class PositionComponent implements OnInit, AfterViewInit {
  ...
  constructor(private page: PageDataService, private spinner: NgxSpinnerService, public dialog: MatDialog, private positionService: PositionService) { }
  ...
  ngOnInit(): void {
    this.page.pagename = "ຂໍ້ມູນຕຳແຫນ່ງງານ"
    this.page.breadcrumb = '<li class="breadcrumb-item"> <i class="fas fa-home"></i> Home</li><li class="breadcrumb-item"> ການຈັດການຂໍ້ມູນພື້ນຖານ </li> <li class="breadcrumb-item active" > ຂໍ້ມູນຕຳແຫນ່ງງານ </li>'
    this.LoadAll();
  }
  ...
  LoadAll() {
    this.positionService.GetList().pipe(this.extractData).subscribe((resp: any) => {
  ...
  doDelete(id: any) {
    this.positionService.Delete(id).subscribe((resp: any) => {
      this.spinner.hide()
      if (resp == "SUCCESS") {
        this.LoadAll();
      }
